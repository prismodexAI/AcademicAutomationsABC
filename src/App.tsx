...
/* === Base path helper (supports Vite BASE_URL, CRA PUBLIC_URL, and subpath deploys) === */
const __BASE__ =
  (typeof import.meta !== 'undefined' && (import.meta as any).env && (import.meta as any).env.BASE_URL)
    ? (import.meta as any).env.BASE_URL
    : (typeof process !== 'undefined' && (process as any).env && (process as any).env.PUBLIC_URL)
      ? (process as any).env.PUBLIC_URL
      : '';

const __asset = (p: string) => `${__BASE__}${p.replace(/^\//, '')}`;




/* === Reports served from /public (no embedded HTML in App.tsx) === */
const SLT_REPORT_SRC = __asset('SLT_Report.html');
const ATTENDANCE_REPORT_SRC = __asset('Attendance_Report.html');
const MERITS_REPORT_SRC = __asset('Merit_Report.html');
const BEHAVIOUR_REPORT_SRC = __asset('Behaviour_Report.html');
